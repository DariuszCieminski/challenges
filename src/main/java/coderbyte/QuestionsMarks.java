...
Have the function coderbyte.QuestionsMarks(str) take the str string parameter, which will contain single digit numbers, letters,
...
public class QuestionsMarks {
    public static String solution(String str) {
        ...
        int pairsEqualToTen = 0;

        for (char c : str.toCharArray()) {
            if (Character.isDigit(c)) {
                //set next pair of numbers
                leftNumber = rightNumber;
                rightNumber = Character.getNumericValue(c);
                if ((leftNumber + rightNumber) == 10) {
                    if (questionMarksCounter != 3) {
                        return "false";
                    }
                    pairsEqualToTen++;
                }
                //reset counter when the new pair is set
                questionMarksCounter = 0;
            } else if (c == '?') {
                questionMarksCounter++;
            }
        }
        //if there weren't any number pair equal to 10, return false
        return (pairsEqualToTen > 0) ? "true" : "false";
    }
}